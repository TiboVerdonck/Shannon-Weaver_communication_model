...
def encode(codebook, message) -> str:
    """
    Encode the message (list of symbols) based on the codebook
    """
    return "".join([codebook[symb] for symb in message])

def decode(tree:Tree, message) -> list:
    ...
    while ptr <= len(message):
        if ptr < len(message):
            bit = message[ptr]
        if type(n) is Leaf:
                decoded_message.append(n.symbol)
                n = root
                
        else:
            n = n.left if bit is "0" else n.right
            ptr+=1
